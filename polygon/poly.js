...
var q0,q1,q2
var t = 0
var obj = Math.floor(Math.random()*5)
...
function axisIcosa(axis) {
  ...
  var pt3 = rotate(pt2,rota(axis[0]),72)
  var pt4 = rotate(pt3,rota(axis[0]),72)
  var pt5 = rotate(pt4,rota(axis[0]),72)
  var pt6 = conj(pt0)
  var pt7 = rotate(pt6,rota(axis[2]),ang)
  var pt8 = rotate(pt7,rota(axis[0]),72)
  var pt9 = rotate(pt8,rota(axis[0]),72)
  var pt10 = rotate(pt9,rota(axis[0]),72)
  var pt11 = rotate(pt10,rota(axis[0]),72)
...
var shape = createAxis()
var rx = 0
var ry = 0
var rot = [1,Math.random()*6,Math.random()*6,0]
console.log(shape, rot);
...
function project (shape) {
  ...
  if (spin==1) {
    ...
    rx = rx/1.01
    ry = ry/1.01
    var rot = [1,rx/20,ry/20,0]
    console.log(shape, rot);
    var shape = rotateAxis(shape,rot)}
  ctx.lineWidth = 7;
  ctx.lineCap = 'round';
...
function drawEdge(q0,q1) {
  var s =1.8
  var x0= W/2+q0[2]*W/4*s; var y0= H/2-q0[1]*W/4*s;
  var x1= W/2+q1[2]*W/4*s; var y1= H/2-q1[1]*W/4*s;
...
function rotate(quart,axis,rotation) {
  ...
  var sp = Math.tan( (1/360)* 2*Math.PI)
  var rot = [1,axis[1]*sp,axis[2]*sp,axis[3]*sp]
